...
function setGrid(num) {
    // function implementation for grid size
    ...
    for (let i = 0; i < num; ++i) {
        ...
        for (let j = 0; j < num; ++j) {
            ...
            rowDiv.addEventListener('mouseover', function (event) {
                // Change rowDiv color upon click

                if (isClicked) {
                    // color hovered cells while mousedown
...
window.addEventListener('load', (event) => {
    // Set grid on first page load and subsequent reloads                                     // Set to blackmode as default
    blackMode()
    setGrid(value)
})
